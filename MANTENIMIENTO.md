...
|---|---|---|---|---|
| 21-may-2026 | T-32 | Portada (`index.html`) — dos ajustes de texto: firma footer `duendes.app 2026` → `Colmena 2026`; subtítulo header `Bahía de Banderas · Jalisco · Nayarit` → `Jalisco y Nayarit` (opción b con "y", coincide con tarjeta ISABI). Texto puro, sin JS ni motor. Enlace CalcPro del footer (T-31) intacto — comillas anidadas verificadas | `d9eecfe` | CD05 Senior + CD06 Junior |
| 21-may-2026 | T-31 | Portada (`index.html`) reducida de 4 a 2 tarjetas — elimina tarjetas IVA Notarial y CalcPro del grid. CalcPro pasa a enlace discreto en footer (opacidad 0.5 + hover). `iva.html` y `calcpro.html` siguen vivos y accesibles por URL directa (solo se ocultan de portada — decisión Arquitecto opción a, NO borrar). Solo `index.html` tocado. Renumerado de T-25 (HO original CD04) por colisión con nota TC+1; protocolo nuevo: CD04 Supervisor describe tarea sin número, numeración canónica la asigna quien ejecuta contra esta bitácora | `d498c8a` | CD05 Senior + CD06 Junior |
| 21-may-2026 | T-30 | Panel comisión% (`#comisionPctPanel`) cierra con click-outside — espejo de T-29 para el segundo panel (mismo problema UX: única salida sin monto = flechita). Opción A (duplicar, no generalizar) + A1 (guard lee `display`, sin variable de estado — decisión Arquitecto). Nuevo `cerrarComisionPct()`; listener propio; fila con `id="comisionPctToggleRow"`; switch `CLICK_OUTSIDE_CIERRA_COMISION`. Coexiste con listener USD (T-29) sin interferencia. **Hallazgo V11:** el estado de coexistencia (ambos paneles abiertos) ES alcanzable vía llamadas programáticas directas a los toggles (sin click que burbujee → listeners no intervienen); validado con datos que un click afuera desde ese estado cierra AMBOS limpiamente. Inicialmente se infirió "imposible por diseño" — la verificación demostró que sí es alcanzable y está cubierto. Dos switches independientes (USD + comisión%), desactivables por separado. Validar tacto celular | `082841c` | CD05 Senior + CD06 Junior |
| 21-may-2026 | T-29 | Panel USD cierra con click-outside (mejora UX móvil) — antes solo cerraba con flechita o botón "usar monto" (disabled sin monto), única salida molesta en celular. Nuevo `cerrarUSD()` extrae lógica de cierre duplicada; listener `document.click` cierra al tocar fuera (sin limpiar campos); guard `CLICK_OUTSIDE_CIERRA_USD` para desactivar sin revertir. Fila USD con `id="usdToggleRow"` — guard por ID, inmune a orden DOM (hay 2 `.usd-toggle-row`: USD 467 y comisión% 605). Abrir comisión% cierra USD (intencional). Bugs clásicos descartados con datos: abre-cierra por propagación (3 puntos medición) y listener múltiple. Validar tacto en celular | `e10efc2` | CD05 Senior + CD06 Junior |
...
| T-29 | ~~Panel USD cierra con click-outside (UX móvil)~~ | ✅ CERRADO `e10efc2` · 21-may-2026 |
